...
import { COLLECTION_PREFIX, collectionKey, dataKey, dataPrefix, decodePart } from "./keys";
import { batch, closeDb, delValue, entriesByPrefix, getValue, openDb, putValue } from "./storage";
...
export class RocksValthera extends ActionsBase {
    ...
    constructor(public location: string, public opts?: RocksOpenOptions) {
        super();
    ...
    async find(query: VQueryT.Find) {
        ...
        const matched = (await this._collectionEntries(query.collection))
            .map(entry => matchObj(query, entry.data) ? updateFindObject({ ...entry.data }, query.findOpts || {}) : null)
    ...
    async findOne(query: VQueryT.FindOne) {
        ...
        for (const entry of await this._collectionEntries(query.collection)) {
            if (matchObj(query, entry.data))
                return updateFindObject({ ...entry.data }, query.findOpts || {}) as DataInternal;
        }
    ...
    async update(query: VQueryT.Update) {
        ...
        for (const entry of await this._collectionEntries(query.collection)) {
            ...
            const next = updateObj(query, { ...entry.data }) as DataInternal;
            if (entry.data._id !== undefined)
                next._id = entry.data._id;

            ops.push({ type: "put", key: entry.key, value: JSON.stringify(next) });
            results.push(next);
        }
    ...
    async removeCollection(collection: string) {
        const ops: RocksBatchOp[] = [
            { type: "del", key: collectionKey(collection) },
            ...(await entriesByPrefix(this.db, dataPrefix(collection))).map(([key]) => ({ type: "del" as const, key })),
        ];
